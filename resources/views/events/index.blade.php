...
<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
    ...
    <img src="{{ asset('storage/' . $event->image) }}" alt="{{ $event->name }}" class="w-full h-48 object-cover">
        ...
        <h2 class="text-xl font-bold text-purple-900 dark:text-purple-400">{{ $event->location }}</h2>
        <p class="text-gray-700 dark:text-gray-300">{{ $event->created_at }}</p>

        <p class="text-gray-700 dark:text-gray-300">{{ Str::limit($event->description, 50) }}</p>
